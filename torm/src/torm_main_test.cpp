...
void initParameters(torm::TormParameters &params_, int endPose_size, bool debug_visual){
    if (debug_visual){
        params_.planning_time_limit_ = 3.0;
    }else{
        params_.planning_time_limit_ = 3.0;
    }
    ...
    params_.traj_generation_iter_ = 40; // # of Ik candidates
    params_.time_duration_ = 10000;
}
...
int main(int argc, char** argv) {
    ...
    path_load(argv[2], trajectory); // "/data/ready_to_show/random_64-torm-0.csv"
    ...
    Eigen::MatrixXd Mtraj;
    Mtraj.resize(trajectory.size(), num_dof);

    for(int i=0; i<trajectory.size(); i++){
        for(int j=0; j<num_dof; j++){
            Mtraj(i,j) = trajectory[i][j];
        }
    }

    debug->show(targetPoses, Mtraj, 1, 2, true);
...
//        ROS_INFO_STREAM("[TORM] interpolation time: " << (ros::WallTime::now() - wt));
//    }
//
...
////    ROS_INFO_STREAM("[PIRL] feasiblity.. vel: " << opt.checkJointVelocityLimit() <<
////    ", col: " << opt.isCurrentTrajectoryCollisionFree() << ", singularity: " << opt.checkSingularity());
