...
EvalProblemGenerator::EvalProblemGenerator(int n_samples, std::string planning_group, planning_scene::PlanningScenePtr planning_scene, torm::TormIKSolver& iksolver)
:n_samples_(n_samples), planning_group_(planning_group), planning_scene_(planning_scene), iksolver_(iksolver){
    ...
    rnd_pitch_ = std::normal_distribution<double>(0.698, 0.872); // m: 40 deg, std: 50 deg
    rnd_roll_ = std::uniform_real_distribution<double>(-3.1415926535897, 3.1415926535897);

    collectEEPoses();
}

EvalProblemGenerator::~EvalProblemGenerator(){
}

bool EvalProblemGenerator::interpolate_linear(std::vector<std::vector<double>>& wps_list, double interval_length,
                                              std::vector<std::vector<double>>& path,
                                              std::vector<std::vector<double>>& configs){
    for(int i=0; i < wps_list.size(); i++){
        wps_list[i][0] -= fk_base_position_[0];
        wps_list[i][1] -= fk_base_position_[1];
        wps_list[i][2] -= fk_base_position_[2];
    }

    interpolator6D ITP6D(wps_list, interval_length, "linear");
    path_total_n_ = ITP6D.get_total_n();
    path_total_len_ = ITP6D.get_total_len();
    path = ITP6D.get_result();
    configs.reserve(path_total_n_);
    return checkExistIKwSolution(path, configs);
}
bool EvalProblemGenerator::interpolate_spline(std::vector<std::vector<double>>& wps_list, double interval_length,
                                              std::vector<std::vector<double>>& path,
                                              std::vector<std::vector<double>>& configs){
    for(int i=0; i < wps_list.size(); i++){
        wps_list[i][0] -= fk_base_position_[0];
    ...
    interpolator6D ITP6D(wps_list, interval_length, "spline");
    path_total_n_ = ITP6D.get_total_n();
    path_total_len_ = ITP6D.get_total_len();
    path = ITP6D.get_result();
    configs.reserve(path_total_n_);
...
bool EvalProblemGenerator::getRandomProblem(int n_wps, double interval_length,
                                            std::vector<std::vector<double>>& path,
                                            std::vector<std::vector<double>>& configs){
    ...
    interpolator6D ITP6D(test_wps, interval_length, "spline");
    path_total_n_ = ITP6D.get_total_n();
    path_total_len_ = ITP6D.get_total_len();
    path = ITP6D.get_result();
    configs.reserve(path_total_n_);
    return checkExistIK(path, configs);
}

bool EvalProblemGenerator::checkExistIKwSolution(std::vector<std::vector<double>>& path,
                                        std::vector<std::vector<double>>& configs){
    ...
    for(int i=0; i<path.size(); i++){
        ...
        if(i==0) {
            if (!iksolver_.ikSolverCollFree(f, q, 100)) {
                ROS_WARN_STREAM("[EvalProblemGenerator::checkExistIK] cannot find collfree ik. fail at i=" << i << "/" << path.size());
                return false;
            }
        }else{
            if (!iksolver_.ikSolverCollFree(q_cur, f, q)){
                ROS_WARN_STREAM("[EvalProblemGenerator::checkExistIK] cannot find collfree ik. fail at i=" << i << "/" << path.size());
                return false;
            }
        }
...
bool EvalProblemGenerator::checkExistIK(std::vector<std::vector<double>>& path,
                                        std::vector<std::vector<double>>& configs){
    for(int i=0; i<path.size(); i++){
        KDL::JntArray q(n_dof_);
        KDL::Frame f;
...
void EvalProblemGenerator::setCollisionChecker() {
...
bool EvalProblemGenerator::collision_check(KDL::JntArray& rnd_q)
{
...
int EvalProblemGenerator::getTargetEEN(){
    return path_total_n_;
}

double EvalProblemGenerator::getTargetEELen(){
    return path_total_len_;
}


void EvalProblemGenerator::collectEEPoses(){
