...
class PirlEnv_Fetch:
    def __init__(self, sceneType="", rewardType="", obsType="", t=None, vaeLogPath="", zDim=None, pObs=None, zStocastic=None):
    ...
    def setup_problem(self, env_num, prob_path, gap, set_from_start):
        ...
        if self.sceneType == "obs":
            if env_num == -1:
                ...
                if torch.cuda.is_available():
                    occ = occ.cuda()
                self.envLatent = self.vae.encode_reparameterize(occ, stocastic=self.z_stocastic)[0].tolist()
            else:
                occ_name = self.BASE_DIR + "obs/scene_occ/occ_{}.npy".format(env_num)
